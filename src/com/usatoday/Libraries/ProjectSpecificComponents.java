...
//********************************************************************************************************************
	...
	public void VerifyTestEnv(String browser, String Env, String url) throws Exception
	{
		if(browser.equalsIgnoreCase("firefox"))
		{
			...
			webdriver.manage().timeouts().implicitlyWait(5, TimeUnit.SECONDS);//vj
			webdriver.get(url);
			//Thread.sleep(10000);
			this.closebreakingnews(webdriver);//vj
			this.closeTransistionads(webdriver);//vj
			}
		...
		else if(browser.equalsIgnoreCase("chrome"))
		{
			if(Env.equalsIgnoreCase("production"))
			{
		      ...
		      final String userProfile= "C:\\Users\\vikasj\\AppData\\Local\\Google\\Chrome\\User Data\\Default\\";
			...
			{
				this.RunAutoit();
				String userProfile= "C:\\Users\\vikasj\\AppData\\Local\\Google\\Chrome\\User Data\\Default\\Extensions\\Default\\Extensions";
				ChromeOptions options = new ChromeOptions();
				options.addArguments("user-data-dir="+userProfile);
				options.addArguments("--start-maximized");
...
//		act.moveToElement(dest).build().perform();
	...
	public boolean powerrankingsPageHeader(String input)
	{
		boolean retval;
		Pattern pattern;
		Matcher matcher;
		...
		pattern = Pattern.compile(REGEX);
		String reg = input.substring(40, 50);
		System.out.println(reg);
		matcher = pattern.matcher(reg);
		
	/*	if(input.split(REGEX)
		{
...
//					this.closesliderads(driver);
	...
	{
		Reports report = Testexecutor.report;
		String tplr = driver.findElement(By.xpath("//table[@data-category='"+data+"']/descendant::tbody/tr[not(contains(@class,'inactive'))][1]/td[2]")).getText();
	...
	{
		Reports report = Testexecutor.report;
		String tstats = driver.findElement(By.xpath("//table[@data-category='"+data+"']/descendant::tbody/tr[not(contains(@class,'inactive'))][1]/td[contains(@class,'sorting')]")).getText();
	...
	public boolean Scrfinalcards(WebDriver driver, String date) throws Exception
	{
		Reports report = Testexecutor.report;
		boolean retval=true;
		List<WebElement> pscrcards = driver.findElements(By.xpath("//div[contains(@class,'post-event')]"));
	...
	public boolean dispofldrmodule(WebDriver driver, String date) throws Exception
	{
		Reports report = Testexecutor.report;
		boolean retval=true;
		List<WebElement> pscrcards = driver.findElements(By.xpath("//div[contains(@class,'post-event')]"));
	...
	public boolean contextlinkclick(WebDriver driver, String t1, String t2, String link, String data) throws Exception
	{
		Reports report = Testexecutor.report;
		boolean retval=false;
		
	
			WebElement ele = driver.findElement(By.xpath("//div[@class='card-full-width size-full']//article//div[1]"));
			...
			String team1 = driver.findElement(By.xpath("//div[@class='card-full-width size-full']//div[1]/ul/li[3]//ul[1]//li[1]")).getText();
			String team2 = driver.findElement(By.xpath("//div[@class='card-full-width size-full']//div[1]/ul/li[3]//ul[1]//li[3]")).getText();
			System.out.println(team1.concat(team2));
...
//				Thread.sleep(5000);
				list.get(i).click();
				this.waitforOverlayLoad(webdriver);
				this.verifyClickConnectionErrors(webdriver, data);
				retval=true;
...
//		this.closesliderads(driver);
	...
	{
		String locxpath = "//section[contains(@class,'sports card-loading')]";
		...
		try
		{
			...
			if(element)
			{
				System.out.println("Page Load event closed");
			}
		}
		catch(Exception e)
		{
			System.out.println("Page load event not found");
		}
	}
...
//***************To Compare the Rankings Status in the data table in Rankings Page******************//	
	public Boolean Rankingsstats(WebDriver driver, String xpath, String arr[]) throws Exception
	{
		Reports report = Testexecutor.report;
		Boolean retval=true;
		List<WebElement> elements = driver.findElements(By.xpath(xpath));
		List<WebElement> ranks = driver.findElements(By.xpath("//div[contains(@class,'sp-stats-leader-block')]/descendant::h3[@class='sp-stats-leader-subcategory']"));
		for(int i=0; i<elements.size();i++)
		{
			String kpi = ranks.get(i).getText();
			if(elements.get(i).getText().equalsIgnoreCase(arr[i]))
			{
				String Passlog= "PASS - "+kpi+" leader stats displayed as '"+elements.get(i).getText()+"' and matched with expected value '"+arr[i]+"'";
				report.writePassLog(Passlog);
	...
	public boolean PhotoModulechker(WebDriver driver, String xpath, String data) throws Exception
	{
		Reports report = Testexecutor.report;
		boolean retval=true;
		try
		{
			driver.findElement(By.xpath(xpath));
			String Passlog="PASS - Element '"+data+"' is present";
			report.writePassLog(Passlog);
			driver.findElement(By.xpath(xpath)).click();
			this.waitforOverlayLoad(webdriver);
			
					
			List<WebElement> photos  = driver.findElements(By.xpath("//div[@class='gallery-photo-border']"));
			String[] phtocnt = new String[photos.size()];
			
			if (photos.size() > 0)
			{
			String Passlog1="PASS- Photos are displayed in the Gallery";
			report.writePassLog(Passlog);
			}
			
			else {
				String Faillog="FAIL- Photos are not displayed in the Gallery";
				report.writeFailLog(Faillog);
			}
		...
		catch(Exception e)
		{
			...
			Testexecutor.res=false;
			String Faillog="FAIL - Element '"+data+"' is not present";
	...
	public void verifyClickConnectionErrors(WebDriver driver, String data) throws Exception
	{
		Reports report = Testexecutor.report;
		ProjectSpecificComponents pjc = new ProjectSpecificComponents();
		if(pjc.VerifyConnectionErrs(driver))
		{
			...
			report.writeFailLog(Faillog);
			pjc.CloseConnectionErrs(driver);
		}
		else
		{
